...
As the consulting product and design lead for Womp, I helped shape and ship this update.

Womp has been expanding Spark beyond its role as an assistant. The goal was to remove the friction between having an idea and having a 3D-printable object. That meant adding image and 3D mesh generation directly into the chat interface.

Shipping AI for Womp was no easy win. For Womp, AI isn't a gimmick or an optimization. It's about making 3D creation accessible to people who never thought they could — but also not leaving behind creators who want depth, not black-box magic.
...
So Womp built generation directly into Spark. Now you can ask Spark to create an image, then immediately ask it to convert that image to a 3D mesh, then send that mesh to print — all in the same conversation. The content lives in your chat history, and you can iterate on it just like you would iterate on any other part of your design process.

The goal wasn't to replace the tools power users love, but to add a new path into 3D for everyone. Generate objects instantly, then use Womp's editor to compose scenes, customize, and make it your own. Some creators were frustrated with the first AI release — and Womp is listening. Womp's mission is easy 3D for all, not shortcuts that limit control.
...
The models aren't Womp's — the product uses off-the-shelf generation APIs. Your prompts and images aren't used to train anything. That was important from the start.
...
The tricky part isn't the generation itself — it's making sure what comes out the other end will actually print. The models had to be trained specifically to meet Womp's printing requirements. A 3D model that looks good in a preview but won't print isn't helpful.
...
Generation costs money to run, so Womp built a credit system. Free users get 300 credits per day. Pro users get 12,000 credits per month. If you need more, you can buy additional credits.
...
The workflow problem isn't unique to Womp. Most 3D design tools require you to be an expert in both 3D modeling and whatever tool you're using. The learning curve is steep, and the tools are complex. Womp has been trying to lower that barrier, but generation in Spark is the first time there's been a real shortcut.
...
This update isn't the end point. It's a step toward making 3D creation more accessible. Womp is building AI tools that will help users create 3D content without needing to master complex tools. The complexity is still there — it's just hidden behind a more intuitive interface.
