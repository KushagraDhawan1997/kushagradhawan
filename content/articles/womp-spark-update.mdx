...
The team has been working on expanding Womp Spark beyond its role as an assistant. The goal was to remove the friction between having an idea and having a 3D-printable object. That meant adding image and 3D mesh generation directly into the chat interface.

Shipping AI for Womp was no easy win. For the team, AI isn't a gimmick or an optimization. It's about making 3D creation accessible to people who never thought they could — but also not leaving behind creators who want depth, not black-box magic.
...
The tricky part isn't the generation itself — it's making sure what comes out the other end will actually print. The team had to make sure the models were trained specifically to meet Womp's printing requirements. A 3D model that looks good in a preview but won't print isn't helpful.
...
Generation costs money to run, so the team built a credit system. Free users get 300 credits per day. Pro users get 12,000 credits per month. If you need more, you can buy additional credits.
...
This update isn't the end point. It's a step toward making 3D creation more accessible. The team is building AI tools that will help users create 3D content without needing to master complex tools. The complexity is still there — it's just hidden behind a more intuitive interface.
