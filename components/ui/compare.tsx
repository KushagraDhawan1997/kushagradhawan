...
export const Compare = ({
  ...
  return (
    ...
    >
        ...
        <AnimatePresence mode="popLayout">
          {firstImage ? (
            <motion.div
              ...
              transition={{ duration: 0 }}
            >
              <img alt="first image" src={firstImage} className={cn("absolute inset-0  z-20 rounded-2xl shrink-0 w-full h-full select-none", firstImageClassName)} draggable={false} />
            </motion.div>
          ) : null}
        </AnimatePresence>
      </div>
      ...
      <AnimatePresence mode="popLayout">
        {secondImage ? <motion.img className={cn("absolute top-0 left-0 z-[19]  rounded-2xl w-full h-full select-none", secondImageClassname)} alt="second image" src={secondImage} draggable={false} /> : null}
      </AnimatePresence>
    </div>
  );
};
